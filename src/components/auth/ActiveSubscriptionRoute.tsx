...
export function ActiveSubscriptionRoute() {
  ...
  if (hasPendingSubscriptionReview(apiResponse)) {
    return (
      <Navigate
        to={PENDING_REVIEW_PATH}
        replace
        state={{ from: location, reason: 'pending_review' as const }}
      />
    );
  }
  ...
  if (!hasActiveSubscription(apiResponse)) {
    return (
      <Navigate
        ...
        replace
        state={{ from: location, reason: 'subscription_required' as const }}
      />
    );
  }
